...
tsk_fs_open_img(TSK_IMG_INFO * a_img_info, TSK_OFF_T a_offset,
    TSK_FS_TYPE_ENUM a_ftype )
{
	return tsk_fs_open_img_partial( a_img_info, a_offset, a_ftype, -1 );
}
TSK_FS_INFO *
tsk_fs_open_img_partial(TSK_IMG_INFO * a_img_info, TSK_OFF_T a_offset,
    TSK_FS_TYPE_ENUM a_ftype, int partial)
{
    ...
    if (a_ftype == TSK_FS_TYPE_DETECT) {
        ...
        for (i = 0; i < sizeof(FS_OPENERS)/sizeof(FS_OPENERS[0]); ++i) {
            if ((fs_info = (i ? 
                    FS_OPENERS[i].open(a_img_info, a_offset, FS_OPENERS[i].type, 1) : 
                    ntfs_open(a_img_info, a_offset, FS_OPENERS[i].type, 1, partial))
                ) != NULL) {
                // fs opens as type i
                if (fs_first == NULL) {
        ...
        return ntfs_open(a_img_info, a_offset, a_ftype, 0, partial);
    }
